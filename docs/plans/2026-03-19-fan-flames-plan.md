# Fan-Flames Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a skill in workspace-jj that orchestrates parallel subagent execution across isolated jj workspaces, reunifies results via `jj squash --from`, and reports plan coverage.

**Architecture:** Single skill file (`fan-flames.md`) with YAML frontmatter and structured instructions. The skill delegates agent dispatch to superpowers skills and only handles jj-specific lifecycle: workspace creation, change reunification, conflict handling, and plan coverage reporting. A CLAUDE.md override entry routes `subagent-driven-development` through fan-flames in jj repos.

**Tech Stack:** Markdown (skill file), jj CLI, Claude Code Agent tool with `isolation: "worktree"`

**Spec:** `docs/specs/2026-03-18-permission-gateway-and-fan-flames-design.md` (Part 2)
...
├── skills/
│   └── fan-flames.md              # NEW: orchestration skill
CLAUDE.md                          # MODIFY: add superpowers override entry
```

---

### Task 1: Create the Fan-Flames Skill File

**Files:**
- Create: `plugins/workspace-jj/skills/fan-flames.md`

This is the core deliverable — a skill prompt that Claude follows to orchestrate the full lifecycle.

- [ ] **Step 1: Create the skills directory**

```bash
mkdir -p plugins/workspace-jj/skills
```

- [ ] **Step 2: Write the skill file**

Create `plugins/workspace-jj/skills/fan-flames.md` with the following content:
...
Check that the YAML frontmatter is valid and the file is readable:

```bash
head -8 plugins/workspace-jj/skills/fan-flames.md
```

Expected: frontmatter with `---` delimiters, `name:` and `description:` fields.

- [ ] **Step 4: Describe the change**

```bash
jj describe -m "feat(workspace-jj): add fan-flames skill for parallel workspace orchestration"
```

---

### Task 2: Add CLAUDE.md Superpowers Override Entry

**Files:**
- Modify: `CLAUDE.md`

Add the override entry so `subagent-driven-development` routes through fan-flames in jj repos.
...
Locate the superpowers override table.

- [ ] **Step 2: Add the fan-flames override entry**

Add a new row to the existing override table:

```markdown
| `subagent-driven-development` | `workspace-jj:fan-flames` | jj-native workspace isolation with reunification |
```

The table should now have two entries:
...
```bash
jj describe -m "feat(workspace-jj): add fan-flames skill and CLAUDE.md superpowers override

- Add fan-flames.md skill for parallel workspace orchestration
- Route subagent-driven-development through fan-flames in jj repos"
```

---

### Task 3: Update workspace-jj README

**Files:**
- Modify: `plugins/workspace-jj/README.md`

Add fan-flames to the workspace-jj documentation.

- [ ] **Step 1: Read current README**

```bash
cat plugins/workspace-jj/README.md
```

- [ ] **Step 2: Add fan-flames section**

Add after the existing content, before any closing sections:
...
```bash
jj describe -m "feat(workspace-jj): add fan-flames skill, CLAUDE.md override, and README docs

- Add fan-flames.md skill for parallel workspace orchestration
- Route subagent-driven-development through fan-flames in jj repos
- Document fan-flames in workspace-jj README"
```
...
Check that the skill appears in the workspace-jj plugin structure:

```bash
ls -la plugins/workspace-jj/skills/fan-flames.md
```

- [ ] **Step 2: Verify CLAUDE.md override table**

```bash
cat CLAUDE.md
```

Confirm both override entries are present.

- [ ] **Step 3: Verify YAML frontmatter parses correctly**

```bash
# Extract frontmatter and verify name/description
sed -n '/^---$/,/^---$/p' plugins/workspace-jj/skills/fan-flames.md | head -10
```

- [ ] **Step 4: Verify jj commands in the skill are correct**

Spot-check the key jj commands referenced in the skill:

```bash
# Verify squash --from syntax is valid
jj squash --help | grep -A2 '\-\-from'

# Verify workspace forget syntax
jj workspace forget --help 2>&1 | head -5
```

- [ ] **Step 5: Final describe**

```bash
jj describe -m "feat(workspace-jj): fan-flames — parallel workspace orchestration skill
